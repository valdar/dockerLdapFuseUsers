...
EOF
  ...
  #insert initial fuse users data
  INIT_DOMAIN=$(echo ${LDAP_DOMAIN} | cut -f1 -d.)
  INIT_EXTENSION=$(echo ${LDAP_DOMAIN} | cut -f2 -d.)
  status "importing initial fuse user data"
  ldapmodify -w ${LDAP_ADMIN_PASSWORD} -D "cn=admin,dc=${INIT_DOMAIN},dc=${INIT_EXTENSION}" -H ldapi:/// -f /osixia/slapd/initData/fuseUsers.ldif
  ldappasswd -w ${LDAP_ADMIN_PASSWORD} -D "cn=admin,dc=${INIT_DOMAIN},dc=${INIT_EXTENSION}" -H ldapi:/// -s fuseldap "uid=fuseldap,ou=People,dc=example,dc=org"
  ldappasswd -w ${LDAP_ADMIN_PASSWORD} -D "cn=admin,dc=${INIT_DOMAIN},dc=${INIT_EXTENSION}" -H ldapi:/// -s notfuseldap "uid=notfuseldap,ou=People,dc=example,dc=org"
